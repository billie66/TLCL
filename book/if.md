...
executes successfully. If we display the value of the parameter dddd, we see that it is zero.
We execute the ls command a second time, producing an error and examine the
parameter dddd again. This time it contains a 2, indicating that the command encountered
...
在这个例子中，我们执行了两次ls命令。第一次，命令执行成功。如果我们显示参数dddd的值，我们
看到它是零。我们第二次执行ls命令的时候，产生了一个错误，并再次查看参数dddd。这次它包含一个
